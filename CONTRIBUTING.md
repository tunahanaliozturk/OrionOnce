# Contributing to OrionOnce

Thanks for taking the time to look at this. HTTP idempotency for ASP.NET Core: an Idempotency-Key middleware that replays the stored response for retried requests, rejects in-flight duplicates, and detects key reuse with a different request body. The project is small and the bar for contributions is "does it make the package clearer, faster, or safer without expanding the public surface needlessly."
...
```bash
git clone https://github.com/tunahanaliozturk/OrionOnce
cd OrionOnce
dotnet restore
